...
왼쪽으로 치우쳐진 종속변수를 Tukey's Ladder of Powers 방법으로 데이터를 최대한 정규분포에 가깝게 변환시켜 주었다. lambda 값으로는 0 보다 큰 0.15가 나왔으므로 변수들에 0.15 제곱을 해주었다. 그리고 독립변수와 종속변수가 최대한 선형관계가 되도록 종속변수에 적용된 lambda값으로 제곱하여 변환시켜주었다.
