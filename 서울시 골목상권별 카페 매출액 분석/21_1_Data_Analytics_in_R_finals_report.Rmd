---
title: "주성분 회귀분석을 통한 서울시 골목상권별 카페 매출액 분석"
...
저희 조는 이번 기말 보고서의 주제를 ‘코로나 전과 후 서울시 우리마을가게 상권배후지 데이터를 이용하여, 서울시 상권들 중 골목상권 별 카페의 총 매출액의 예측’으로 정했습니다. 
저희 조는 지난 중간 보고서 때 ‘코로나 전과 후 청와대 국민 청원 중 자영업자들의 청원에 유의한 감성차이가 있는가’로 분석을 했습니다. 이번 기말 분석 때 중간 분석 때의 주제와 연결되는 주제로 정했습니다. 이와 같이 정한 이유는, 중간 분석 때 주제와 연관되면서 좀 더 세부적인 분석을 진행하고 싶다는 생각이 들었기 때문입니다. 중간 분석 때는 코로나 19 발생 이전과 이후의 자영업자들 청원 감성에 유의한 차이가 있는지를 알아냈다면, 이번 기말 분석 때는 서울시 상권들 중 골목상권 별 카페의 총 매출액에 어느 정도의 차이가 있는지와 카페 총 매출액에 영향을 미치는 독립변수들이 코로나 발생 전후로 차이가 있는 지 알아보았습니다. 
사용한 데이터는 서울열린데이터 광장에 공시되어 있는 서울시 상권데이터 중 국내 코로나 확진자가 나온 시점에서 1년 전인 2019년부터 1년 후인 2020년까지의 데이터를 수집하였습니다. 저희 조는 <상권 배후지-소득분위>, <상권 배후지-집객시설>, <상권 배후지-상권변화>, <상권 배후지-매출금액>, <상권 배후지-유동인구>, <상권 배후지-상주인구>, <상권–직장인수>로 총 7가지 데이터를 수집하였습니다. 
수집한 데이터에 대한 EDA에 대해서는 필요하지 않은 EDA들을 삭제와 수정하여 최대한 필요한 EDA들로만 생성을 했습니다. 그리고 EDA는 모델 구축 후에 시각화 부분에 대한 참고자료 목적으로 부록으로 따로 마지막 부분에 넣었습니다. 얻은 인사이트를 토대로 매출액에 영향을 끼칠 것으로 판단되는 변수들을 추렸습니다. 이후 추린 변수들을 바탕으로 다중회귀분석을 수행함으로써 어떤 변수들이 매출액에 영향을 끼쳤는지 알아보았습니다.
이번 분석의 한계로는 homoscedasticity인 등분성 가정을 만족하지 못하여, 높은 신뢰도의 결과를 내지 못했다는 점입니다. 그리고 상권의 예상 매출액만을 대상으로 분석을 진행했다는 것입니다. 실제로 상권의 흥망에 영향을 주는 변수들을 정확하게 찾기 위해선 상권의 지출액 등 여러가지 요소 또한 고려해 봐야할 것입니다. 
...
위와 같은 데이터들을 수집한 후, 수집한 데이터 파일 중에서 독립변수로 사용할 변수들과, 종속변수로 사용할 변수들에 대한 전처리를 수행했고 코로나 발생후인 2020년 2- 4분기까지의 데이터와 마찬가지로 2019년 2-4분기 데이터만을 필터링하여 사용했습니다.


# iii. 분석 주제 (연구문제 설정)

## iii.i 서울시 골목상권 별 카페 총 매출액에 관련된 주제를 선정한 이유 & 목표
...
#### 2.1.3. 2019년과 2020년의 변수 별 매출금액 평균 - 주중, 주말
...
#### 2.1.4. 2019년과 2020년의 변수 별 매출금액 평균 - 요일
...
#### 2.1.5. 2019년과 2020년의 변수 별 매출금액 평균 - 시간대
...
#### 2.1.6. 2019년과 2020년의 변수 별 매출금액 평균 - 성별
...
#### 2.1.7. 2019년과 2020년의 변수 별 매출금액 평균 - 연령대
...
### 2.2. 점포수 대비 매출액이 높은 상권과 낮은 상권 비교
