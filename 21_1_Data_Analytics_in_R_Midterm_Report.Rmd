...
저희 조는 이번 중간 보고서의 주제를 ‘코로나 전과 후 국민 청원들 중 자영업 키워드와 관련된 청원에 유의한 감성 차이가 있는가’ 로 정했습니다. 

코로나19 발생 이전과 이후의 차이가 있는 것을 주제로 삼고 싶었고, 특히 코로나19는 특별히 자영업자 분들에게 막대한 경제적 피해를 입히고 있기 때문에 청와대 국민 청원 게시글에도 영향이 있을 것이라 생각했습니다. 따라서 코로나19 전후로 자영업/소상공인 키워드를 기준으로 청원 게시글을 분석하기로 하였고, 분석을 통해 코로나 19 이후 자영업자들의 불만이 어떻게 더 증가하였는지 알아보았습니다. 

국내 코로나 확진자가 나온 시점에서 1년 전인 2019/01/20 부터 1년 후인 2021/01/20 까지의 국민 청원 게시글들을 크롤링 하여 수집했습니다.  

수집한 데이터를 가지고 간단하계 기술통계를 진행했습니다. 여기에서 얻은 데이터를 토대로 본 분석 주제인 자영업, 소상공인 키워드로 데이터를 좁혀 나갔습니다. 코로나 전과 후로 자영업, 소상공인 키워드로만 추출한 데이터에 어떠한 차이가 있는지 분석을 했습니다. 

텍스트 분석을 통해 코로나 이전과 이후 자영업, 소상공인 키워드의 워드클라우드를 그려 단어의 빈도수를 확인했습니다. 코로나 이후의 워드클라우드 에서는 ‘지원’ 단어 글자의 크기가 커졌고, ‘학원`, `마트`, `긴급`, `재난`, `방역` 같은 단어들이 새롭게 나타났습니다. 그리고 네트워크 맵을 그려 단어들 간의 연관성을 확인했습니다.  

그런 다음, 청원 게시글의 타이틀에 나타난 감성 차이에 대해서 분석해보았습니다. 결과를 보았을 때, 코로나 전후로 부정적인 감성이 약 3%가 증가했습니다. 이 3%의 차이가 통계적으로 유의한 차이가 될 수 있는지 추가적인 검정을 진행했습니다. 

감성 점수를 토대로 추론통계를 하기 위해 정규성 검정을 진행했습니다. 데이터가 정규분포를 따르지 않아 Wilcoxon rank sum test로 검정을 진행한 결과, 자영업, 소상공인 관련된 국민 청원 게시글의 감성이 코로나 이전보다 이후에 더 나빠졌다는 대립가설이 채택되었습니다.

따라서 자영업, 소상공인과 관련된 국민 청원 게시글의 감성이 코로나 이전보다 이후에 더 부정적으로 변했다고 해석할 수 있었습니다. 
부정적으로 감성이 변한 이유에 대해서 생각해보았을때 사회적 거리두기, 5인 이상 사적 모임 금지, 영업 시간 단축과 같은 행정 조치들로 인해 국민 청원 게시판을 통해 어려움을 호소하고 있다고 판단됩니다. 
...
국내 코로나 확진자가 나온 시점에서 1년전인 2019/1/20 부터 1년 후인 2021/1/20 까지의 국민 청원 게시글에 청원번호(id) , 청원 카테고리(category), 청원 제목(title), 청원 만료 날짜(expiryDate), 청원 동의수(numOfAgrees) 총 5가지 데이터를 수집하였다. 총 85,856개의 청원 게시글을 수집할 수 있었습니다. 
...
같은 주제이지만 청원 게시자가 임의로 카테고리를 설정하면서 발생한 상황으로 해석됩니다. 이는 카테고리로만 분석을 진행할시 완전한 데이터를 얻지 못하게 될 가능성을 높임으로 새로운 방침이 필요해 보입니다.
...
속성에 단어들이 들어가 있고 값에 단어들에 대한 상관계수가 들어갔다. 

TF-IDF에 대한 상관행렬을 대상으로 `network()`함수를 사용하여 네트워크 객체를 생성하였다. 그 후 상관계수를 통해 상관행렬을 크기를 조정하여 edge의 개수를 줄여 그래프의 가시성을 높였다. 상관계수는 그래프의 가시성을 가장 높일 수 있는 계수로 설정하였다. 
...
*매개 중심성*은 노드가 얼마나 단어와 단어를 잘 연결해주는지를 나타내는 용어이다. 매개 중심성은 sna패키지의 betweenness함수를 활용하여 구했다. 
해당 그래프는 다른 노드들과 연결이 많이 되어있는 노드 증 상위 10%에 해당하는 단어들을 금색으로 표현할 수 있도록 설정하였다.  
...
감성분석을 통해 구한 게시글별 감정 점수를 사용하여 코로나 전후로 국민 청원 게시글의 감성이 더 나빠졌는지 알아보고자 합니다. 
...
즉 자영업, 소상공인 관련 국민 청원 게시글의 감성이 코로나 이전보다 이후에 더 나빠졌다고 해석할 수 있습니다.

---

# 7. 결론

이번 분석을 통해 코로나19 전과 후로 청와대 국민 청원 게시판에 게시되는 청원들에 어떤 차이가 있었는지와 사회적 트렌드의 변화를 확인할 수 있었습니다. 그리고 자영업/소상공인에 관한 국민 청원 게시글들의 성격을 확인해보았습니다. 코로나19 이전에도 해당 키워드의 청원들은 상당히 부정적이었습니다. 하지만 코로나19 이후 사회적 거리두기, 5인 이상 사적 모임 금지, 영업 시간 단축과 같은 여러 행정 조치들로 인해 정부 또는 행정기관을 향한 자영업자들의 감정은 더욱 안좋아졌습니다. 
