...
국내 코로나 확진자가 나온 시점에서 1년전인 2019/1/20 부터 1년 후인 2021/1/20 까지의 국민 청원 게시글에 청원번호(id) , 청원 카테고리(category), 청원 제목(title), 청원 만료 날짜(expiryDate), 청원 동의수(numOfAgrees) 총 5가지 데이터를 수집하였다. 총 85,856개의 청원 게시글을 수집할 수 있었다. 
...
크롤링은 셀레니움을 사용해 진행 하였으며 수집한 데이터는 데이터프레임에 저장하고 날짜 데이터와 청원 동의수 데이터는 각각 날짜 타입의 데이터, 정수 타입의 데이터로 변환하여 주었다. 그리고 2020/1/20일 기준으로 코로나 이전과 이후 데이터프레임을 생성하여 데이터를 두개로 나누었다. 코로나 이전 데이터는 총 75,162개의 청원 게시글이 있고, 코로나 이후 데이터는 총 10,694개의 청원 게시글이 있다. 코로나 이전 1년동안 게시된 청원의 갯수가 코로나 이후 1년동안 게시된 청원의 갯수보다 대 7.5배 차이가 남을 알 수 있다. 
...
수집한 데이터를 그래프들을 통해 코로나 이전과 이후 데이터가 어떤 모습인지 알아보았다. 

## 3.1 청원 동의수 top 5

코로나 이전에 동의수를 가장 많이 받은 청원 5개와 코로나 이후에 동의수를 가장 많이 받은 청원 5개를 확인할 수 있었다. 코로나 이전에는 정치에 관한 청원들이 동의를 많이 얻었음을 알 수 있다. 코로나 이후에는 n번방 사건에 대한 청원과 정치와 관련된 청원들이 많은 동의를 얻었다. 
...
title <- text_grob("카테고리별 청원 동의수", family = "NanumGothic", size = 16, face = "bold")
...
국민 청원 게시글에는 게시자가 지정할 수 있는 카테고리가 17가지 있다. 카테고리별로 청원 게시글 갯수와 전체 청원 게시글 갯수와의 비율을 구하여 아래 그래프를 사용하여 나타냈다. 
...
```
'안전/환경' 카테고리의 비율이 굉장히 높아진걸 알 수 있고 그에 반해 '농산어촌' 카테고리는 비율이 많이 줄은것을 알 수 있다. 코로나로 인해 '안전/환경' 카테고리에 이전보다 많은 청원들이 올라왔을 것이라 생각된다.

분석은 코로나 전과 후로 자영업 소상공인 키워드로 추출한 데이터에 어떠한 차이가 있는지 알아보고자 한다
...
자영업과 소상공인 키워드로 청원 게시글들을 추출 하였다.
...
# 전체 '자영업/소상공인' 키워드 청원 건수와 동의수 데이터
total_df %>% head
```
...
또한 '자영업/소상공인'과 관련된 청원 게시글에 어떤 차이가 있는지 그래프를 통해 알아보았다. 먼저 코로나 이전과 이후로 청원 동의수의 차이를 알아보았고, 다음으로 코로나 이전과 이후로 청원 건수의 차이를 알아보았다.
...
청원 게시글 텍스트 데이터에서 의미없는 단어는 삭제시키기 위해 불용어 사전을 사용하였다. 그리고 추가로 불용어라 판단되는 단어들을 새로운 불용어 사전에 추가하여 텍스트 분석을 진행 하였다.
...
# beforeDtmTfIdf[[1]]$dimnames$Terms
# 형태소 분석 결과값
beforeWords <- beforeTextData[2]

afterTextData <- textAnalysis(after_cat)
afterDtmTfIdf <- afterTextData[1]
# afterDtmTfIdf[[1]]$dimnames$Terms
afterWords <- afterTextData[2]
```

## 4.5 워드클라우드

워드클라우드는 빈도수가 높은 순서대로 글자의 크기와 색이 결정되어 어떤 단어들이 데이터에 많이 반복되어 사용되고 있는지 알 수 있다.

형태소 분석을 통해 생성한 문서 단어 행렬 데이터를 이용하여 워드클라우드를 그려보았다. 
...
```{r message=F}
# 가중치가 적용된 단어 빈도
wc1 <- wc(beforeDtmTfIdf[[1]])
# word로 knitr할시 제대로 나오지 않아 워드클라우드 figure을 html로 내보낸후 스크린샷을 찍어 word에 첨부
saveWidget(wc1,"wc1.html",selfcontained = F)
webshot("wc1.html","wc1.png", delay =5, vwidth = 480, vheight=480)
```
...
```{r message=F}
# 가중치가 적용된 단어 빈도
wc2 <- wc(afterDtmTfIdf[[1]])
saveWidget(wc1,"wc2.html",selfcontained = F)
webshot("wc2.html","wc2.png", delay =5, vwidth = 480, vheight=480)
```

대체적으로 부정적인 단어가 많이 나타나는것을 알 수 있다. 

코로나 이전과 이후의 워드클라우드에서 찾아볼 수 있는 차이점은 코로나 이후에는 "지원"이란 단어의 크기가 커졌고, "정책", "긴급'', "노래방" 등 의 단어들이 새로 나타났다. 
...
감성사전을 불러와서 긍정사전에는 단어당 1점, 부정사전에서는 단어당 -1점을 부여 하였다. 이후 형태소 분석을 해서 나온 데이터를 토크나이징 한 다음 각 단어에 대해 감성점수를 부여 하였다. 

단어 별로 감성점수를 구한 후 그걸 다시 하나의 문장으로 합치는 과정에서 단어당 감성점수를 모두 더하여 청원 게시글 한개의 감성 점수를 구하였다.

코로나 이전과 이후의 감성 점수를 비교하기 위해 중립은 제외하고 긍부정 점수만을 비율로 나타낸 후 파이차트를 그려보았다.
...
# 감성 비율 그래프 출력
ggarrange(beforeFig, afterFig, labels = c("before covid 19", "after covid 19"), hjust = c(-0.9,-1.2), vjust = 3) %>% 
  annotate_figure(top=text_grob("코로나 전후 청원 게시글 감성", face="bold", family="NanumGothic", size=24))
```

코로나 이전에는 부정적인 감성이 70.4%를 차지했고 긍정적인 감성이 29.6%를 차지했다.  

코로나 이후에는 부정적인 감성이 73.76% 이고 긍정적인 감성이 26.24%이다. 

부정적인 감성이 코로나 전후로는 약 3% 증가하였으며 긍정적인 감성은 약 3% 감소했다. 이제 이 차이가 통계적으로 유의한 차이인지 검정해보고자 한다.

---

# 6. 추론통계

감성분석을 통해 구한 게시글별 감정 점수를 사용하여 코로나 전후로 국민 청원 게시글의 감성이 더 나빠졌는지 알아보고자 한다. 
...
감성점수 데이터에 대한 정규성 검정을 진행하였다. qqplot과 히스토그램을 그려보고, 코로나 이전과 이후 청원 게시들의 감성점수들이 정규분포를 따르는지 대해 shapiro.test를 통해서도 정규성을 검증해 보았다. 
...
코로나 이후의 감성점수가 더 나빠졌으니 wilcox.test의 alternative에 "greater"이라는 값을 주어 우측검정을 실시한다.
...
코로나19는 대면 서비스업을 직업으로 하는 수많은 사람들에게 경제적 피해를 일으켰다. 그중 자영업자들의 피해가 가장 크게 나타나고 있다.

코로나19 이전에도 자영업/소상공인에 관한 국민 청원 게시글의 감성은 상당히 부정적이었다. 하지만 코로나19 이후 사회적 거리두기, 5인 이상 사적 모임 금지, 영업 시간 단축과 같은 여러 행정 조치들로 인해 자영업자들은 이전 보다 더욱 어려움을 호소하고 있고 이는 국민의 목소리를 직접적으로 듣고자 개설된 청와대 국민 청원 게시판의 코로나19 전과 후로 '자영업/소상공인' 키워드로 검색된 게시글들의 감성 점수의 차이로 확인할 수 있었다. 